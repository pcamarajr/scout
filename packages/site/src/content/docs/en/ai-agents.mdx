---
title: "AI agents & MCP"
description: "Scout is built to be driven by the agent that wrote the feature: it scaffolds onboarding files for coding agents, runs a co-author loop, and exposes an MCP server — with a verification-integrity rule that forbids claiming a pass without a real run."
...
Scout's main flow is meant to be driven **by the agent that built the feature**: the agent writes the scenario in natural language (never the script — the script is born from the verified execution) and triggers verification. This is the loop closing on itself — the same agent that produces the code is held to the outcome it promised.
