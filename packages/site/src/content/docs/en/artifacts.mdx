---
title: "Run artifacts"
description: "What each run records — trace, screenshots, report, structured result, transcript — and the opt-in preview video that turns a verified flow into a GitHub-playable clip for PR review."
...
- Enable per-run with `--record-video`, via `SCOUT_RECORD_VIDEO=1`, or `"recordVideo": true` in the config (see the [config file reference](/docs/config/)).
