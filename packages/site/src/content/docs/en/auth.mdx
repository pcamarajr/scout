...
description: "Run authenticated flows with a browser session captured once per environment, pick a profile per scenario, and keep secrets out of the committed script with $ENV: placeholders."
sidebarLabel: "Auth profiles & secrets"
order: 12
group: "Reference"
tags: ["auth", "profiles", "secrets"]
translationKey: "auth"
lang: en
draft: false
---
...
A scenario picks a profile via file-level `profile:` frontmatter or a per-scenario override (see [Writing scenarios](/docs/scenarios/)). For the full set of config keys, see the [config file reference](/docs/config/).
...
> `scout login` captures **your application's** logged-in session — it is **not** AI/model-provider auth. For that, see [Providers & credentials](/docs/providers/) and `scout doctor`.
