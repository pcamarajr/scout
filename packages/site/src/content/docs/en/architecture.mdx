---
title: "Architecture"
description: "How Scout works inside — the module layout, the design decisions that keep recorded scripts honest, why a runner failure is never an app verdict, and the known limitations."
...
│   │                       #   pooled process → per-run context, ref→locator resolution
│   ├── selector-ladder.ts  # record-time selector preference ladder + fragility + fallbacks
...
- **The agent never writes test code.** It acts in the browser; each interaction is recorded by walking a **selector preference ladder** against the live element (`data-testid` → hand-authored `id` → role + *computed* accessible name → visible text → positional CSS) and keeping the sturdiest strategy that uniquely matches. When only a positional path is left the step is flagged **fragile** and warned at record time; the other matching strategies are kept as ordered **fallbacks** that replay tries deterministically (no LLM) before failing. Eliminates hallucinated selectors — and keeps them stable as the DOM drifts. See [Stable selectors](/docs/how-verification-works/#stable-selectors).
- **Assertions are tools.** The agent registers each expectation via `browser_assert` (DOM/URL), `browser_assert_network` (an API call happened, with tolerant method/URL/status/body matching), `browser_assert_no_console_errors`, and `browser_assert_state` (an element's visual/structural state — class token, attribute, or computed style, so an `opacity:0` show/hide toggle that Playwright would count as *visible* is checked correctly) — that's what makes the replay a real test, not just a click macro.
- **Some elements have no accessible ref.** `browser_click` acts on numbered refs from the a11y snapshot; for a role-less element (a gesture layer, an overlay `<div data-testid="…">`) the agent falls back to `browser_click_selector`, which records a `data-testid`-first (CSS fallback) locator — no hallucinated selectors, still a deterministic click.
...
- **One browser process, many contexts.** Chromium is launched once and pooled per launch config (headless/`slowMo`) for the life of the process; each run gets a fresh isolated `BrowserContext` — the real isolation boundary for storage state, cookies, trace, and video. A single scenario used to pay for up to four browser launches (cached replay, heal, demo replay, fallback); they now share one process, and the pool is torn down on the CLI exit paths so it never keeps the event loop alive.
- **Trace for debugging, video for humans.** `trace.zip` is the deep-debug artifact; the opt-in preview video is a low-friction, GitHub-playable clip for PR review. See [run artifacts](/docs/artifacts/).
