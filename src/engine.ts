  ...
  if (cached?.length) {
    ...
    if (replay.passed) {
      result = {
        ...
        mode: "replay",
        verdict: "verified",
        reason: `Script determinístico passou (${cached.length} steps).`,
      ...
      result = {
        ...aiResult,
        healed: true,
        reason: `Script cacheado quebrou no step ${replay.failedIndex! + 1} (${replay.failedStep}: ${replay.error}). Re-verificado por AI: ${aiResult.reason}`,
        startedAt,
        durationMs: Date.now() - start,
      };
    } else {
      result = {
        ...
        mode: "replay",
        verdict: "failed",
        reason: `Step ${replay.failedIndex! + 1} falhou: ${replay.failedStep} — ${replay.error}${heal ? " (AI heal indisponível: sem credenciais Anthropic)" : " (heal desligado)"}`,
        stepCount: cached.length,
    ...
    if (!aiAvailable()) {
      throw new Error(
        `Cenário "${scenario.slug}" não tem script gravado e não há credenciais Anthropic (ANTHROPIC_API_KEY) para o run AI inicial.`
      );
    }
    const aiResult = await runAi(scenario, config, store, runDir, storageState, opts);
